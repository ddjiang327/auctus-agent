...
Features:
- System tray icon with context menu (Windows/Linux only on macOS due to compatibility)
...
class SystemTray:
    ...
    def create_icon(self) -> None:
        ...
        # Note: pystray has compatibility issues with pywebview on macOS
        # causing "trace trap" crashes. Disable on macOS for now.
        if sys.platform == 'darwin':
            print("[Tray] System tray disabled on macOS due to compatibility issues.")
            self.tray = None
            return
        ...
        try:
            ...
            self.tray = pystray.Icon("auctus_agent", icon_image, "Auctus Agent", menu)
...
class GlobalHotkey:
    ...
    def start(self) -> None:
        try:
            ...
            self.listener = keyboard.GlobalHotKeys({'+'.join(keys): on_activate})
            self.listener.start()
            self._running = True
...
class NotificationManager:
    ...
    def send(self, title: str, message: str, timeout: int = 5) -> None:
        ...
        try:
            ...
            notification.notify(title=title, message=message, app_name="Auctus Agent", timeout=timeout)
...
class AuctusDesktopEnhanced:
    ...
    def start_backend(self) -> None:
        ...
        self.server_process = subprocess.Popen(
            [python_exe, '-m', 'uvicorn', 'app.server:app', '--host', '127.0.0.1', '--port', str(self.port), '--log-level', 'warning'],
    ...
    def on_window_closing(self) -> None:
        if self.tray and self.tray.tray:
            ...
            self.window.hide()
            self.notifications.send("Auctus Agent", "Running in background. Use tray icon or hotkey to restore.")
            return False
        else:
            self.shutdown()
    ...
    def start_enhanced_features(self) -> None:
        ...
        if self.notifications:
            self.notifications.send("Auctus Agent Started", "Press Cmd/Ctrl+Shift+A to toggle window.")
